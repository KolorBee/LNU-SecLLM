...
void SPARKFastLIO2::pclPointIMUToBase(PointType const *const pi, PointType *const po) {
  const auto offset_R_B_I = latest_state_.offset_R_L_I * lidar_R_wrt_base_.inverse();
...
void SPARKFastLIO2::integrateIMU(esekfom::esekf<state_ikfom, 12, input_ikfom> &state,
  ...
  const int64_t t0 = rclcpp::Time(imu_queue[0].header.stamp).nanoseconds();
  const int64_t t1 = rclcpp::Time(imu_queue[1].header.stamp).nanoseconds();

  if (t1 <= t0) {
    RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 1000,
                         "Dropping non-ascending IMU integration pair (%ld -> %ld ns)",
                         t0, t1);
    auto latest = imu_queue.back();
    imu_queue.clear();
    imu_queue.push_back(latest);
    return;
  }

  const double dt = static_cast<double>(t1 - t0) * 1.0e-9;
  if (!std::isfinite(dt) || dt > 0.2) {
    RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 1000,
                         "Dropping abnormal IMU integration interval %.6f s", dt);
    auto latest = imu_queue.back();
    imu_queue.clear();
    imu_queue.push_back(latest);
    return;
...
PoseStruct SPARKFastLIO2::transformPoseWrtLidarFrame(const state_ikfom &state) const {
  Eigen::Vector3d lidar_position = state.pos + state.rot * state.offset_T_L_I;
  Eigen::Quaterniond lidar_orientation = state.rot * state.offset_R_L_I;
...
PoseStruct SPARKFastLIO2::transformPoseWrtBaseFrame(const state_ikfom &state) const {
  ...
  Eigen::Vector3d base_position = state.pos + state.rot * offset_T_B_I;
  Eigen::Quaterniond base_orientation(state.rot.toRotationMatrix() * offset_R_B_I);
