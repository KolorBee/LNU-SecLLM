...
void Preprocess::hesai_handler(const sensor_msgs::msg::PointCloud2 &msg) {
  ...
  pl_surf.reserve(plsize / std::max(1, point_filter_num) + 1);
  const double first_timestamp = pl_orig.front().timestamp;
  const double scan_period_ms  = 1000.0 / std::max(1, SCAN_RATE);
  const bool valid_first_timestamp =
      std::isfinite(first_timestamp) && first_timestamp > 0.0;
  ...
  for (size_t i = 0; i < plsize; i++) {
    ...
    const double offset_ms = (source.timestamp - first_timestamp) * time_unit_scale;
    if (valid_first_timestamp && std::isfinite(source.timestamp) && offset_ms >= 0.0 &&
        offset_ms <= scan_period_ms * 2.0) {
      added_pt.curvature = static_cast<float>(offset_ms);
    } else {
      added_pt.curvature =
          static_cast<float>(scan_period_ms * static_cast<double>(i) / plsize);
    }
    pl_surf.points.push_back(added_pt);
  }
}
