...
本文档说明 LNU-SecLLM Web 前端（`web/templates/index.html` + `web/static/js/*.js`）的设计与开发规范，确保在不引入打包工具和不改动后端路由的前提下，实现可扩展、低返工的多语言支持。
...
- **前端主导的客户端**：所有 UI 文案在浏览器端根据当前语言动态渲染，后端 Go 仅负责结构和数据，不参与语言分发。
